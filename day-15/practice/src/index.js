    ...
    return this.state.data.map((country) => {
      ...
      return (
        <div>
          <div>
            {' '}
            <img src={country.flag} alt={country.name[0]} />{' '}
          </div>
          <div>
            <h1>{country.name}</h1>
  ...
  render() {
    return (
      <div className='App'>
        ...
        <p>The time now is {moment(new Date()).format('HH:mm:ss')}</p>
        ...
        <TiSocialTwitterCircular />
        <DiAndroid/>
        <div className={`footer ${this.className}`}>
          <Child className={this.style}></Child>
        </div>
        ...
        <div>
          ...
          <div className='countries-wrapper' styles={footerStyles}>{this.renderCountries()}</div>
        </div>
      </div>
    )
  }
